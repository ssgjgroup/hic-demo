    ...
    while @@FETCH_STATUS = 0 --返回被 FETCH语句执行的最后游标的状态--
      begin
        ...
        select @yljgdm,
               ...
               0,
               '0'
        from (select id, value from [dbo].[f_splitString](@shzdmc, @regex)) _0,
             (select id, value from [dbo].[f_splitString](@shzdbm, @regex)) _1
        where _0.id = _1.id
        ...
        fetch next from order_cursor
        into @yjlxh, @shzdbm, @shzdmc   --转到下一个游标
      end
    if @error = 0
      begin
        commit tran   --提交事务
