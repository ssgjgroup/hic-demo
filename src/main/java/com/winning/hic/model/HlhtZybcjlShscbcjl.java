package com.winning.hic.model;

import java.util.Date;
import java.io.Serializable; 

import org.apache.ibatis.type.Alias; 

import com.winning.hic.model.BaseDomain;



/**
 * @author HLHT
 * @title 
 * @email Winning Health
 * @package com.winning.hic.model
 * @date 2018-34-31 16:34:11
 */
@Alias("hlhtZybcjlShscbcjl")
public class HlhtZybcjlShscbcjl extends BaseDomain implements Serializable {

    private static final long serialVersionUID = -1L;

    /**
     * 字段名：yjlxh
     * 备注: 
     * 默认值：无
     */
    private String yjlxh;
    /**
     * 字段名：jzlsh
     * 备注: 
     * 默认值：无
     */
    private String jzlsh;
    /**
     * 字段名：patid
     * 备注: 
     * 默认值：无
     */
    private String patid;
    /**
     * 字段名：zyh
     * 备注: 
     * 默认值：无
     */
    private String zyh;
    /**
     * 字段名：ksdm
     * 备注: 
     * 默认值：无
     */
    private String ksdm;
    /**
     * 字段名：ksmc
     * 备注: 
     * 默认值：无
     */
    private String ksmc;
    /**
     * 字段名：bqdm
     * 备注: 
     * 默认值：无
     */
    private String bqdm;
    /**
     * 字段名：bqmc
     * 备注: 
     * 默认值：无
     */
    private String bqmc;
    /**
     * 字段名：bfh
     * 备注: 
     * 默认值：无
     */
    private String bfh;
    /**
     * 字段名：bfmc
     * 备注: 
     * 默认值：无
     */
    private String bfmc;
    /**
     * 字段名：bch
     * 备注: 
     * 默认值：无
     */
    private String bch;
    /**
     * 字段名：hzxm
     * 备注: 
     * 默认值：无
     */
    private String hzxm;
    /**
     * 字段名：birth
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp birth;
    /**
     * 字段名：lxdh
     * 备注: 
     * 默认值：无
     */
    private String lxdh;
    /**
     * 字段名：hyzkdm
     * 备注: 
     * 默认值：无
     */
    private String hyzkdm;
    /**
     * 字段名：hyzk
     * 备注: 
     * 默认值：无
     */
    private String hyzk;
    /**
     * 字段名：mzdm
     * 备注: 
     * 默认值：无
     */
    private String mzdm;
    /**
     * 字段名：mz
     * 备注: 
     * 默认值：无
     */
    private String mz;
    /**
     * 字段名：hkszd
     * 备注: 
     * 默认值：无
     */
    private String hkszd;
    /**
     * 字段名：csdz
     * 备注: 
     * 默认值：无
     */
    private String csdz;
    /**
     * 字段名：gzdw
     * 备注: 
     * 默认值：无
     */
    private String gzdw;
    /**
     * 字段名：gzdwdz
     * 备注: 
     * 默认值：无
     */
    private String gzdwdz;
    /**
     * 字段名：gzdwdh
     * 备注: 
     * 默认值：无
     */
    private String gzdwdh;
    /**
     * 字段名：jgd
     * 备注: 
     * 默认值：无
     */
    private String jgd;
    /**
     * 字段名：sfzhm
     * 备注: 
     * 默认值：无
     */
    private String sfzhm;
    /**
     * 字段名：xbdm
     * 备注: 
     * 默认值：无
     */
    private String xbdm;
    /**
     * 字段名：xbmc
     * 备注: 
     * 默认值：无
     */
    private String xbmc;
    /**
     * 字段名：nls
     * 备注: 
     * 默认值：无
     */
    private Short nls;
    /**
     * 字段名：nly
     * 备注: 
     * 默认值：无
     */
    private String nly;
    /**
     * 字段名：jlrq
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp jlrq;
    /**
     * 字段名：ssczbm
     * 备注: 
     * 默认值：无
     */
    private String ssczbm;
    /**
     * 字段名：ssmc
     * 备注: 
     * 默认值：无
     */
    private String ssmc;
    /**
     * 字段名：ssmbbwdm
     * 备注: 
     * 默认值：无
     */
    private String ssmbbwdm;
    /**
     * 字段名：ssbwmc
     * 备注: 
     * 默认值：无
     */
    private String ssbwmc;
    /**
     * 字段名：ssrq
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp ssrq;
    /**
     * 字段名：mzffdm
     * 备注: 
     * 默认值：无
     */
    private String mzffdm;
    /**
     * 字段名：mzffmc
     * 备注: 
     * 默认值：无
     */
    private String mzffmc;
    /**
     * 字段名：ssgc
     * 备注: 
     * 默认值：无
     */
    private String ssgc;
    /**
     * 字段名：shzdmc
     * 备注: 
     * 默认值：无
     */
    private String shzdmc;
    /**
     * 字段名：shzdbm
     * 备注: 
     * 默认值：无
     */
    private String shzdbm;
    /**
     * 字段名：zdyjdm
     * 备注: 
     * 默认值：无
     */
    private String zdyjdm;
    /**
     * 字段名：zdyj
     * 备注: 
     * 默认值：无
     */
    private String zdyj;
    /**
     * 字段名：zysx
     * 备注: 
     * 默认值：无
     */
    private String zysx;
    /**
     * 字段名：ysbm
     * 备注: 
     * 默认值：无
     */
    private String ysbm;
    /**
     * 字段名：ysqm
     * 备注: 
     * 默认值：无
     */
    private String ysqm;
    /**
     * 字段名：qmrq
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp qmrq;
    /**
     * 字段名：jzkssj
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp jzkssj;
    /**
     * 字段名：jzjssj
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp jzjssj;
    /**
     * 字段名：gxsj
     * 备注: 
     * 默认值：无
     */
    private java.sql.Timestamp gxsj;
    /**
     * 字段名：syxh
     * 备注: 
     * 默认值：无
     */
    private String syxh;
    /**
     * 字段名：tjzt
     * 备注: 
     * 默认值：无
     */
    private String tjzt;
    /**
     * 字段名：yxjl
     * 备注: 
     * 默认值：无
     */
    private Integer yxjl;

    public HlhtZybcjlShscbcjl (){

    }

   /**
   * 字段名：yjlxh
   * get方法
   * 备注: 
   */
   public String getYjlxh(){

        return yjlxh;
   }

   /**
   * 字段名：yjlxh
   * set方法
   * 备注: 
   */
   public void setYjlxh(String yjlxh){
        this.yjlxh = yjlxh;
   }
   /**
   * 字段名：jzlsh
   * get方法
   * 备注: 
   */
   public String getJzlsh(){

        return jzlsh;
   }

   /**
   * 字段名：jzlsh
   * set方法
   * 备注: 
   */
   public void setJzlsh(String jzlsh){
        this.jzlsh = jzlsh;
   }
   /**
   * 字段名：patid
   * get方法
   * 备注: 
   */
   public String getPatid(){

        return patid;
   }

   /**
   * 字段名：patid
   * set方法
   * 备注: 
   */
   public void setPatid(String patid){
        this.patid = patid;
   }
   /**
   * 字段名：zyh
   * get方法
   * 备注: 
   */
   public String getZyh(){

        return zyh;
   }

   /**
   * 字段名：zyh
   * set方法
   * 备注: 
   */
   public void setZyh(String zyh){
        this.zyh = zyh;
   }
   /**
   * 字段名：ksdm
   * get方法
   * 备注: 
   */
   public String getKsdm(){

        return ksdm;
   }

   /**
   * 字段名：ksdm
   * set方法
   * 备注: 
   */
   public void setKsdm(String ksdm){
        this.ksdm = ksdm;
   }
   /**
   * 字段名：ksmc
   * get方法
   * 备注: 
   */
   public String getKsmc(){

        return ksmc;
   }

   /**
   * 字段名：ksmc
   * set方法
   * 备注: 
   */
   public void setKsmc(String ksmc){
        this.ksmc = ksmc;
   }
   /**
   * 字段名：bqdm
   * get方法
   * 备注: 
   */
   public String getBqdm(){

        return bqdm;
   }

   /**
   * 字段名：bqdm
   * set方法
   * 备注: 
   */
   public void setBqdm(String bqdm){
        this.bqdm = bqdm;
   }
   /**
   * 字段名：bqmc
   * get方法
   * 备注: 
   */
   public String getBqmc(){

        return bqmc;
   }

   /**
   * 字段名：bqmc
   * set方法
   * 备注: 
   */
   public void setBqmc(String bqmc){
        this.bqmc = bqmc;
   }
   /**
   * 字段名：bfh
   * get方法
   * 备注: 
   */
   public String getBfh(){

        return bfh;
   }

   /**
   * 字段名：bfh
   * set方法
   * 备注: 
   */
   public void setBfh(String bfh){
        this.bfh = bfh;
   }
   /**
   * 字段名：bfmc
   * get方法
   * 备注: 
   */
   public String getBfmc(){

        return bfmc;
   }

   /**
   * 字段名：bfmc
   * set方法
   * 备注: 
   */
   public void setBfmc(String bfmc){
        this.bfmc = bfmc;
   }
   /**
   * 字段名：bch
   * get方法
   * 备注: 
   */
   public String getBch(){

        return bch;
   }

   /**
   * 字段名：bch
   * set方法
   * 备注: 
   */
   public void setBch(String bch){
        this.bch = bch;
   }
   /**
   * 字段名：hzxm
   * get方法
   * 备注: 
   */
   public String getHzxm(){

        return hzxm;
   }

   /**
   * 字段名：hzxm
   * set方法
   * 备注: 
   */
   public void setHzxm(String hzxm){
        this.hzxm = hzxm;
   }
   /**
   * 字段名：birth
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getBirth(){

        return birth;
   }

   /**
   * 字段名：birth
   * set方法
   * 备注: 
   */
   public void setBirth(java.sql.Timestamp birth){
        this.birth = birth;
   }
   /**
   * 字段名：lxdh
   * get方法
   * 备注: 
   */
   public String getLxdh(){

        return lxdh;
   }

   /**
   * 字段名：lxdh
   * set方法
   * 备注: 
   */
   public void setLxdh(String lxdh){
        this.lxdh = lxdh;
   }
   /**
   * 字段名：hyzkdm
   * get方法
   * 备注: 
   */
   public String getHyzkdm(){

        return hyzkdm;
   }

   /**
   * 字段名：hyzkdm
   * set方法
   * 备注: 
   */
   public void setHyzkdm(String hyzkdm){
        this.hyzkdm = hyzkdm;
   }
   /**
   * 字段名：hyzk
   * get方法
   * 备注: 
   */
   public String getHyzk(){

        return hyzk;
   }

   /**
   * 字段名：hyzk
   * set方法
   * 备注: 
   */
   public void setHyzk(String hyzk){
        this.hyzk = hyzk;
   }
   /**
   * 字段名：mzdm
   * get方法
   * 备注: 
   */
   public String getMzdm(){

        return mzdm;
   }

   /**
   * 字段名：mzdm
   * set方法
   * 备注: 
   */
   public void setMzdm(String mzdm){
        this.mzdm = mzdm;
   }
   /**
   * 字段名：mz
   * get方法
   * 备注: 
   */
   public String getMz(){

        return mz;
   }

   /**
   * 字段名：mz
   * set方法
   * 备注: 
   */
   public void setMz(String mz){
        this.mz = mz;
   }
   /**
   * 字段名：hkszd
   * get方法
   * 备注: 
   */
   public String getHkszd(){

        return hkszd;
   }

   /**
   * 字段名：hkszd
   * set方法
   * 备注: 
   */
   public void setHkszd(String hkszd){
        this.hkszd = hkszd;
   }
   /**
   * 字段名：csdz
   * get方法
   * 备注: 
   */
   public String getCsdz(){

        return csdz;
   }

   /**
   * 字段名：csdz
   * set方法
   * 备注: 
   */
   public void setCsdz(String csdz){
        this.csdz = csdz;
   }
   /**
   * 字段名：gzdw
   * get方法
   * 备注: 
   */
   public String getGzdw(){

        return gzdw;
   }

   /**
   * 字段名：gzdw
   * set方法
   * 备注: 
   */
   public void setGzdw(String gzdw){
        this.gzdw = gzdw;
   }
   /**
   * 字段名：gzdwdz
   * get方法
   * 备注: 
   */
   public String getGzdwdz(){

        return gzdwdz;
   }

   /**
   * 字段名：gzdwdz
   * set方法
   * 备注: 
   */
   public void setGzdwdz(String gzdwdz){
        this.gzdwdz = gzdwdz;
   }
   /**
   * 字段名：gzdwdh
   * get方法
   * 备注: 
   */
   public String getGzdwdh(){

        return gzdwdh;
   }

   /**
   * 字段名：gzdwdh
   * set方法
   * 备注: 
   */
   public void setGzdwdh(String gzdwdh){
        this.gzdwdh = gzdwdh;
   }
   /**
   * 字段名：jgd
   * get方法
   * 备注: 
   */
   public String getJgd(){

        return jgd;
   }

   /**
   * 字段名：jgd
   * set方法
   * 备注: 
   */
   public void setJgd(String jgd){
        this.jgd = jgd;
   }
   /**
   * 字段名：sfzhm
   * get方法
   * 备注: 
   */
   public String getSfzhm(){

        return sfzhm;
   }

   /**
   * 字段名：sfzhm
   * set方法
   * 备注: 
   */
   public void setSfzhm(String sfzhm){
        this.sfzhm = sfzhm;
   }
   /**
   * 字段名：xbdm
   * get方法
   * 备注: 
   */
   public String getXbdm(){

        return xbdm;
   }

   /**
   * 字段名：xbdm
   * set方法
   * 备注: 
   */
   public void setXbdm(String xbdm){
        this.xbdm = xbdm;
   }
   /**
   * 字段名：xbmc
   * get方法
   * 备注: 
   */
   public String getXbmc(){

        return xbmc;
   }

   /**
   * 字段名：xbmc
   * set方法
   * 备注: 
   */
   public void setXbmc(String xbmc){
        this.xbmc = xbmc;
   }
   /**
   * 字段名：nls
   * get方法
   * 备注: 
   */
   public Short getNls(){

        return nls;
   }

   /**
   * 字段名：nls
   * set方法
   * 备注: 
   */
   public void setNls(Short nls){
        this.nls = nls;
   }
   /**
   * 字段名：nly
   * get方法
   * 备注: 
   */
   public String getNly(){

        return nly;
   }

   /**
   * 字段名：nly
   * set方法
   * 备注: 
   */
   public void setNly(String nly){
        this.nly = nly;
   }
   /**
   * 字段名：jlrq
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getJlrq(){

        return jlrq;
   }

   /**
   * 字段名：jlrq
   * set方法
   * 备注: 
   */
   public void setJlrq(java.sql.Timestamp jlrq){
        this.jlrq = jlrq;
   }
   /**
   * 字段名：ssczbm
   * get方法
   * 备注: 
   */
   public String getSsczbm(){

        return ssczbm;
   }

   /**
   * 字段名：ssczbm
   * set方法
   * 备注: 
   */
   public void setSsczbm(String ssczbm){
        this.ssczbm = ssczbm;
   }
   /**
   * 字段名：ssmc
   * get方法
   * 备注: 
   */
   public String getSsmc(){

        return ssmc;
   }

   /**
   * 字段名：ssmc
   * set方法
   * 备注: 
   */
   public void setSsmc(String ssmc){
        this.ssmc = ssmc;
   }
   /**
   * 字段名：ssmbbwdm
   * get方法
   * 备注: 
   */
   public String getSsmbbwdm(){

        return ssmbbwdm;
   }

   /**
   * 字段名：ssmbbwdm
   * set方法
   * 备注: 
   */
   public void setSsmbbwdm(String ssmbbwdm){
        this.ssmbbwdm = ssmbbwdm;
   }
   /**
   * 字段名：ssbwmc
   * get方法
   * 备注: 
   */
   public String getSsbwmc(){

        return ssbwmc;
   }

   /**
   * 字段名：ssbwmc
   * set方法
   * 备注: 
   */
   public void setSsbwmc(String ssbwmc){
        this.ssbwmc = ssbwmc;
   }
   /**
   * 字段名：ssrq
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getSsrq(){

        return ssrq;
   }

   /**
   * 字段名：ssrq
   * set方法
   * 备注: 
   */
   public void setSsrq(java.sql.Timestamp ssrq){
        this.ssrq = ssrq;
   }
   /**
   * 字段名：mzffdm
   * get方法
   * 备注: 
   */
   public String getMzffdm(){

        return mzffdm;
   }

   /**
   * 字段名：mzffdm
   * set方法
   * 备注: 
   */
   public void setMzffdm(String mzffdm){
        this.mzffdm = mzffdm;
   }
   /**
   * 字段名：mzffmc
   * get方法
   * 备注: 
   */
   public String getMzffmc(){

        return mzffmc;
   }

   /**
   * 字段名：mzffmc
   * set方法
   * 备注: 
   */
   public void setMzffmc(String mzffmc){
        this.mzffmc = mzffmc;
   }
   /**
   * 字段名：ssgc
   * get方法
   * 备注: 
   */
   public String getSsgc(){

        return ssgc;
   }

   /**
   * 字段名：ssgc
   * set方法
   * 备注: 
   */
   public void setSsgc(String ssgc){
        this.ssgc = ssgc;
   }
   /**
   * 字段名：shzdmc
   * get方法
   * 备注: 
   */
   public String getShzdmc(){

        return shzdmc;
   }

   /**
   * 字段名：shzdmc
   * set方法
   * 备注: 
   */
   public void setShzdmc(String shzdmc){
        this.shzdmc = shzdmc;
   }
   /**
   * 字段名：shzdbm
   * get方法
   * 备注: 
   */
   public String getShzdbm(){

        return shzdbm;
   }

   /**
   * 字段名：shzdbm
   * set方法
   * 备注: 
   */
   public void setShzdbm(String shzdbm){
        this.shzdbm = shzdbm;
   }
   /**
   * 字段名：zdyjdm
   * get方法
   * 备注: 
   */
   public String getZdyjdm(){

        return zdyjdm;
   }

   /**
   * 字段名：zdyjdm
   * set方法
   * 备注: 
   */
   public void setZdyjdm(String zdyjdm){
        this.zdyjdm = zdyjdm;
   }
   /**
   * 字段名：zdyj
   * get方法
   * 备注: 
   */
   public String getZdyj(){

        return zdyj;
   }

   /**
   * 字段名：zdyj
   * set方法
   * 备注: 
   */
   public void setZdyj(String zdyj){
        this.zdyj = zdyj;
   }
   /**
   * 字段名：zysx
   * get方法
   * 备注: 
   */
   public String getZysx(){

        return zysx;
   }

   /**
   * 字段名：zysx
   * set方法
   * 备注: 
   */
   public void setZysx(String zysx){
        this.zysx = zysx;
   }
   /**
   * 字段名：ysbm
   * get方法
   * 备注: 
   */
   public String getYsbm(){

        return ysbm;
   }

   /**
   * 字段名：ysbm
   * set方法
   * 备注: 
   */
   public void setYsbm(String ysbm){
        this.ysbm = ysbm;
   }
   /**
   * 字段名：ysqm
   * get方法
   * 备注: 
   */
   public String getYsqm(){

        return ysqm;
   }

   /**
   * 字段名：ysqm
   * set方法
   * 备注: 
   */
   public void setYsqm(String ysqm){
        this.ysqm = ysqm;
   }
   /**
   * 字段名：qmrq
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getQmrq(){

        return qmrq;
   }

   /**
   * 字段名：qmrq
   * set方法
   * 备注: 
   */
   public void setQmrq(java.sql.Timestamp qmrq){
        this.qmrq = qmrq;
   }
   /**
   * 字段名：jzkssj
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getJzkssj(){

        return jzkssj;
   }

   /**
   * 字段名：jzkssj
   * set方法
   * 备注: 
   */
   public void setJzkssj(java.sql.Timestamp jzkssj){
        this.jzkssj = jzkssj;
   }
   /**
   * 字段名：jzjssj
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getJzjssj(){

        return jzjssj;
   }

   /**
   * 字段名：jzjssj
   * set方法
   * 备注: 
   */
   public void setJzjssj(java.sql.Timestamp jzjssj){
        this.jzjssj = jzjssj;
   }
   /**
   * 字段名：gxsj
   * get方法
   * 备注: 
   */
   public java.sql.Timestamp getGxsj(){

        return gxsj;
   }

   /**
   * 字段名：gxsj
   * set方法
   * 备注: 
   */
   public void setGxsj(java.sql.Timestamp gxsj){
        this.gxsj = gxsj;
   }
   /**
   * 字段名：syxh
   * get方法
   * 备注: 
   */
   public String getSyxh(){

        return syxh;
   }

   /**
   * 字段名：syxh
   * set方法
   * 备注: 
   */
   public void setSyxh(String syxh){
        this.syxh = syxh;
   }
   /**
   * 字段名：tjzt
   * get方法
   * 备注: 
   */
   public String getTjzt(){

        return tjzt;
   }

   /**
   * 字段名：tjzt
   * set方法
   * 备注: 
   */
   public void setTjzt(String tjzt){
        this.tjzt = tjzt;
   }
   /**
   * 字段名：yxjl
   * get方法
   * 备注: 
   */
   public Integer getYxjl(){

        return yxjl;
   }

   /**
   * 字段名：yxjl
   * set方法
   * 备注: 
   */
   public void setYxjl(Integer yxjl){
        this.yxjl = yxjl;
   }

}