...
public class HlhtZybcjlJjbjlServiceImpl implements HlhtZybcjlJjbjlService {
    ...
    public MbzDataCheck interfaceHlhtZybcjlJjbjl(MbzDataCheck t) {
            ...
            for (HlhtZybcjlJjbjl obj : hlhtZybcjlJjbjls) {
                ...
                if (!StringUtil.isEmptyOrNull(obj.getBlnr())) {
                    try {
                        ...
                        String log = Constants.WN_ZYBCJL_JJBJL_SOURCE_TYPE +"||"+getClass().toString()+"||"+"解析病历报错,病历名称：{"+obj.getBlmc()+"},源记录序号{"+obj.getYjlxh()+"}"+"||错误原因:{"+e.getMessage()+"}";
                ...
                try {
                    ...
                    String log = Constants.WN_ZYBCJL_JJBJL_SOURCE_TYPE +"||"+getClass().toString()+"||"+"病历百分比计算报错,病历名称：{"+obj.getBlmc()+"},源记录序号{"+obj.getYjlxh()+"}"+"||错误原因:{"+e.getMessage()+"}";
                    mbzLogService.createMbzLog(log);
                    continue;
                }
                real_count++;
        ...
        //更新dc表
        t.getMap().put("sourceType",Constants.WN_ZYBCJL_JJBJL_SOURCE_TYPE);
