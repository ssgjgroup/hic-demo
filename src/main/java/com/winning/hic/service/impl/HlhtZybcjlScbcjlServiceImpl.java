...
public class HlhtZybcjlScbcjlServiceImpl implements HlhtZybcjlScbcjlService {
    ...
    public MbzDataCheck interfaceHlhtZybcjlScbcjl(MbzDataCheck t) {
        ...
        if (hlhtZybcjlScbcjls != null) {
            ...
            for (HlhtZybcjlScbcjl obj : hlhtZybcjlScbcjls) {
                ...
                try {
                    ...
                    String log = Constants.WN_ZYBCJL_SCBCJL_SOURCE_TYPE +"||"+getClass().toString()+"||"+"解析病历报错,病历名称：{"+obj.getBlmc()+"},源记录序号{"+obj.getYjlxh()+"}"+"||错误原因:{"+e.getMessage()+"}";
                ...
                try {
                    ...
                    String log = Constants.WN_ZYBCJL_SCBCJL_SOURCE_TYPE +"||"+getClass().toString()+"||"+"病历百分比计算报错,病历名称：{"+obj.getBlmc()+"},源记录序号{"+obj.getYjlxh()+"}"+"||错误原因:{"+e.getMessage()+"}";
                    mbzLogService.createMbzLog(log);
                    continue;
                }
                real_count++;
        ...
        //更新dc表
        t.getMap().put("sourceType",Constants.WN_ZYBCJL_SCBCJL_SOURCE_TYPE);
