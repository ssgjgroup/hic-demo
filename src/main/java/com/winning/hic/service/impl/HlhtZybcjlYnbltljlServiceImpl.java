...
public class HlhtZybcjlYnbltljlServiceImpl implements HlhtZybcjlYnbltljlService {
    ...
    @Override
    public MbzDataCheck interfaceHlhtZybcjlYnbltljl(MbzDataCheck t) {
        //获取数据集字典表中配置，判断是否需要抽取
        MbzDictInfo mbzDictInfo = new MbzDictInfo();
        mbzDictInfo.setDictCode("platformTableName");
        mbzDictInfo.setDictValue(Constants.WN_ZYBCJL_YNBLTLJL_SOURCE_TYPE);
        ...
        if (hlhtZybcjlYnbltljlList != null) {
            ...
            for (HlhtZybcjlYnbltljl obj : hlhtZybcjlYnbltljlList) {
                ...
                try {
                    ...
                    String log = Constants.WN_ZYBCJL_YNBLTLJL_SOURCE_TYPE +"||"+getClass().toString()+"||"+"解析病历报错,病历名称：{"+obj.getBlmc()+"},源记录序号{"+obj.getYjlxh()+"}"+"||错误原因:{"+e.getMessage()+"}";
                ...
                try {
                    ...
                    String log = Constants.WN_ZYBCJL_YNBLTLJL_SOURCE_TYPE +"||"+getClass().toString()+"||"+"病历百分比计算报错,病历名称：{"+obj.getBlmc()+"},源记录序号{"+obj.getYjlxh()+"}"+"||错误原因:{"+e.getMessage()+"}";
                    mbzLogService.createMbzLog(log);
                    continue;
                }
                try {
        ...
        //更新dc表
        t.getMap().put("sourceType",Constants.WN_ZYBCJL_YNBLTLJL_SOURCE_TYPE);
